...
impl SimpleDecoder {
    pub fn new_simple() -> SimpleDecoder {
        SimpleDecoder::new(2usize.pow(13), 44100)
    }
...
//        let simple_freqs = (0..sample_count).map(|v| { v * sample_rate / sample_count }).collect();
        let complex_freqs = (0..num_outputs).map(|v| kammer_ton * 2.0_f64.powf(v as f64 / PER_OCTAVE as f64 - 3.0) / sample_rate as f64 * sample_count as f64).map(|v| v as usize);
        println!("complex_freqs: {:?}", complex_freqs);
        SimpleDecoder {
            sample_rate: sample_rate,
            sample_count: sample_count,
            freqs: complex_freqs.filter(|v| *v < sample_count).collect(),
