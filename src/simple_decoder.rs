...
//        let simple_freqs = (0..sample_count).map(|v| { v * sample_rate / sample_count }).collect();
    ...
    pub fn decode(&mut self, input: &[f32]) -> Vec<f32> {
            ...
            for (i, &(index, lower_weight, upper_weight)) in self.freqs.iter().enumerate() {
                let val = self.fft_out[index] * lower_weight + self.fft_out[index+1] * upper_weight  ;
